...
class liteAvatar(object):
    def __init__(self,
        ...
        if self.num_threads > 1:
            # face_gen_loop below runs self.num_threads worker threads,
            # each calling self.generator(...) - a PyTorch forward pass -
            # concurrently, one video frame per call. Left alone, PyTorch
            # ALSO tries to parallelize each individual forward pass
            # across every CPU core internally, so N worker threads doing
            # that at the same time massively oversubscribes the CPU
            # (N threads x all-cores-each) and ends up slower than a
            # single thread, not faster. Restricting torch to one thread
            # per call and getting the actual parallelism from the worker
            # threads instead (each one handling a different frame) is
            # what actually uses the extra cores productively.
    ...
    def _start_workers(self):
        # NOTE: each face_gen_loop worker thread below exits as soon as it
        # sees the None sentinel that marks the end of one handle() call's
        # input (see face_gen_loop) - it does not loop forever waiting for
        # more work. That's fine for a single command-line run, but this
        # object may now be reused for several handle() calls in a row (the
        # web server keeps one liteAvatar per avatar around instead of
        # reloading its weights every request), so a fresh queue + worker
        # pool is spun up at the start of every handle() call rather than
    ...
    def handle(self, audio_file_path, result_dir, param_res=None):
        ...
        if os.path.exists(tmp_frame_dir):
            # was: os.system(f'rm -rf {tmp_frame_dir}') - a Linux-only shell
            # command that silently no-ops on Windows (no rm.exe), which then
            # made the os.mkdir() right below fail with FileExistsError.
            # shutil.rmtree() works the same way on every platform.
        ...
        # was: hardcoded '/usr/bin/ffmpeg', which doesn't exist on Windows.
        # imageio-ffmpeg (already a pinned dependency) ships its own portable
        # ffmpeg binary and works the same on Windows/Mac/Linux, so use that
        # instead of assuming ffmpeg is separately installed and on PATH.
        frame_count = len([f for f in os.listdir(tmp_frame_dir) if f.endswith('.jpg')])
        if frame_count == 0:
            raise RuntimeError(
                'No video frames were generated before the ffmpeg step '
                '(tmp_frames is empty) - the failure happened earlier in '
                'the pipeline, not in ffmpeg itself. Check the console '
                'output above this point for the real error.'
            )

        result_path = os.path.join(result_dir, 'test_demo.mp4')
        # was: os.system(cmd) with a hand-quoted command string - worked
        # most of the time, but silently swallowed ffmpeg's stderr (so
        # failures were invisible unless you were watching the console at
        # the exact right moment) and hand-quoting a command string is
        # fragile on Windows whenever a path contains spaces. A list-based
        # subprocess.run() sidesteps the quoting issue entirely (each
        # argument is passed as-is, no shell involved) and lets us capture
        # and surface the actual error message instead of just noticing
        # the output file is missing.
        cmd = [
            get_ffmpeg_exe(),
            '-r', '30',
            '-i', os.path.join(tmp_frame_dir, '%05d.jpg'),
            '-i', audio_file_path,
            '-framerate', '30',
            '-c:v', 'libx264',
            '-preset', 'veryfast',
        ...
        if proc.returncode != 0 or not os.path.exists(result_path):
            ...
            raise RuntimeError(
                f'ffmpeg failed (exit code {proc.returncode}) while '
                f'assembling the final video.'
